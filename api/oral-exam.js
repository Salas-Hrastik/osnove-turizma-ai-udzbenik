  ...
  if (body.mode === 'answer') {
    ...
    try {
      const result = await callAnthropic(apiKey, model,
        ...
        `Pitanje iz cjeline ${text(body.chapterId, 10)} „${text(body.chapterTitle, 240)}”: ${question}\nOčekivana osnova: ${expectedAnswer}\nStudentov odgovor: ${answer}\nPrivremena procjena prije potvrde završetka: ${body.provisional === true ? 'da' : 'ne'}\nSugestivna pomoć već korištena: ${body.hintUsed === true ? 'da' : 'ne'}\nProcijeni točnost, obuhvat, povezivanje pojmova i jasnoću. Ocjena score mora biti cijeli broj 0–20. complete je true samo ako je odgovor dovoljno sadržajan da ga se može konačno vrednovati; kod privremene procjene bitno nedovršenog odgovora postavi complete false i oblikuj jedno kratko sugestivno pomoćno pitanje koje usmjerava, ali ne otkriva cijeli odgovor. Vrati JSON: {"score":number,"complete":boolean,"level":"kratka razina","strengths":"jedna do dvije rečenice","omissions":"jedna do dvije rečenice","feedback":"kratko usmeno obrazloženje studentu","hint":"jedno kratko pomoćno pitanje"}.`,
        700)
